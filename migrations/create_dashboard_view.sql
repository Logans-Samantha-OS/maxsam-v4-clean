...
-- Using correct table names from our migrations

CREATE OR REPLACE VIEW dashboard_summary AS
SELECT 
    -- Pipeline Overview
    (SELECT COUNT(*) FROM leads WHERE COALESCE(status, 'new') NOT IN ('closed', 'opted_out')) as total_active_leads,
    (SELECT COUNT(*) FROM leads WHERE COALESCE(phone, phone_1, phone_2) IS NOT NULL AND COALESCE(status, 'new') NOT IN ('closed', 'opted_out')) as leads_with_phones,
    (SELECT COUNT(*) FROM leads WHERE status IN ('contacted', 'enriched', 'scored') AND COALESCE(phone, phone_1, phone_2) IS NOT NULL) as ready_for_outreach,
