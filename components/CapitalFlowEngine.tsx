...
export default function CapitalFlowEngine() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/contracts/metrics", {
          cache: "no-store",
        });
  ...
  if (loading) {
    return <div className="text-sm text-gray-400">Loading capital metrics…</div>;
  }
