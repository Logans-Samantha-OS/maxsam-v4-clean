...
import SmsBlastButton from '@/components/SmsBlastButton';
                    ...
                    <table className="pharaoh-table">
                      ...
                      <tbody>
                        {hotLeads.map((lead) => {
                          ...
                          const attempts = lead.call_attempts || 0; // Using call_attempts as proxy for contact_attempts based on existing interface
                            ...
                            <tr key={lead.id}>
                              ...
                              <td className="text-center">
                                <div className="flex flex-col items-center">
                                  {attempts > 0 ? (
                                    <>
                                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${lead.status === 'qualified' ? 'bg-green-500/20 text-green-400' :
                                        lead.status === 'contacted' ? 'bg-cyan-500/20 text-cyan-400' :
                                          lead.status === 'dead' ? 'bg-red-500/20 text-red-400' :
                                            'bg-zinc-700 text-zinc-400'
                                        }`}>
                                        {lead.status === 'qualified' ? 'RESPONDED' :
                                          lead.status === 'dead' ? 'OPT-OUT' :
                                            `${attempts} attempt${attempts !== 1 ? 's' : ''}`}
                                      </span>
                                      <span className="text-[10px] text-zinc-500 mt-0.5">
                                        {getRelativeTime(lead.last_call_date)}
                                      </span>
                                    </>
                                  ) : (
                                    <span className="text-zinc-600 text-xs italic">No outreach</span>
                                  )}
                                </div>
                              </td>
