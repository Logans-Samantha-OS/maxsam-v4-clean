...
export async function POST(request: NextRequest) {
  ...
  try {
    if (await isSystemPaused()) {
      return NextResponse.json({ success: false, error: 'System is paused. Dispatch routes are temporarily disabled.' }, { status: 423 })
    }
    ...
    if (!resolvedLeadId) {
      ...
      for (const lead of leads || []) {
        if (normalizePhone(lead.phone) === normalized) {
